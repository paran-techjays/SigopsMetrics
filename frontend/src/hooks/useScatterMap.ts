...
export const useScatterMap = () => {
  ...
  const processMapData = useCallback((signals: Signal[], metricData: MetricDataItem[], mapConfig: MapConfig) => {
    setLoading(true);
    try {
      console.log("Processing map data with config:", mapConfig);
      console.log("Signals:", signals.length, "Metric data:", metricData.length);
      ...
      for (let i = 0; i < mapConfig.ranges.length; i++) {
        ...
        console.log(`Range ${i} (${range[0]}-${range[1]}):`, rangeSignals.length, "signals");
      ...
      console.log("Created traces:", mapTraces.length);
