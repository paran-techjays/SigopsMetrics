...
export const fetchZonesByZoneGroup = createAsyncThunk(
  ...
  async (zoneGroup: string, { rejectWithValue }) => {
    try {
      console.log(`Fetching zones for zone group: ${zoneGroup}`);
      const result = await filterApi.getZonesByZoneGroup(zoneGroup);
      console.log(`Zones by zone group response:`, result);
...
const filterSlice = createSlice({
  ...
  reducers: {
    ...
    saveAsDefaults: (state) => {
      console.log('Saving current filters as defaults');
      const filters = {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchZones.pending, (state) => {
      ...
      console.log('Loading all zones - pending');
    });
    builder.addCase(fetchZones.fulfilled, (state, action) => {
      ...
      if (Array.isArray(action.payload) && action.payload.length > 0) {
        ...
        console.log('All zones loaded successfully:', action.payload.length, 'zones');
      } else {
    ...
    builder.addCase(fetchZonesByZoneGroup.pending, (state) => {
      state.loading.zones = true;
      console.log('Loading filtered zones - pending');
    });
    builder.addCase(fetchZonesByZoneGroup.fulfilled, (state, action) => {
      ...
      if (Array.isArray(action.payload) && action.payload.length > 0) {
        state.zones = action.payload;
        console.log('Filtered zones loaded successfully:', action.payload.length, 'zones');
      } else {
